...
ASSEMBLE_TABLE = [
    ...
    ["first", "second", 0, {"first_second": ["third"]}, "random jibberish ",
        "random jibberish", ],
    ["first", "second", 3, {"first_second": ["third"]}, "random jibberish ",
...
COMPLEX_INPUT_FILES = [
    [
        ...
        " Scarlet, I was seized with a keen desire to see Holmes again," +
        " and to know how he was employing his extraordinary powers. His" +
        " rooms were brilliantly lit, and, even as I looked up, I saw his" +
        " tall, spare figure pass twice in a dark silhouette against the" +
        " blind. He was pacing the room swiftly, eagerly, with his head" +
        " sunk upon his chest and his hands clasped behind him. To me," +
        " who knew his every mood and habit, his attitude and manner" +
        " told their own story. He was at work again. He had risen out of" +
...
def test_read_file(file_name, result):
    """Test create_dct function #1.

    Must return a trigram dictionary from list of words.
    """
...
@pytest.mark.parametrize("first, second, num, dct, text, result",
                         ASSEMBLE_TABLE)
def test_assemble(first, second, num, dct, text, result):
    """Test assemble function.

    Must recursively generate random words from dictionary.
    """
    from trigrams import assemble
    assert assemble(dct, num, text, first, second) == result


@pytest.mark.parametrize("first, second, num, dct, text",
                         ASSEMBLE_TABLE_RANDOM)
...
def test_create_story(input_file, result):
    ...
    Test with simple files to create a specific new story using trigrams.
    """
    import trigrams
...
def test_create_story_complex(input_file, result):
    ...
    Test with complex files to create a new story of the same style
    using trigrams.
    """
